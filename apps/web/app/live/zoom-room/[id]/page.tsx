    ...
    return (
      <div className="mx-auto max-w-3xl space-y-4 px-4 py-10">
        ...
        <Button asChild variant="outline" className="border-slate-700 bg-slate-950 text-slate-200">
          <Link href="/live/zoom/hosting">Back to my Zoom host classes</Link>
        </Button>
      </div>
    );
  }
      ...
      <header className="flex flex-wrap items-start justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-3">
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-xs text-slate-400">
            {new Date(session.scheduled_at).toLocaleString()} · Zoom control room
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          {isAdmin ? (
            <Button asChild variant="outline" size="sm" className="border-slate-700 bg-slate-950 text-slate-200">
              <Link href={`/live/zoom/admin/${id}`} target="_blank" rel="noreferrer">
            ...
            <Button asChild variant="outline" size="sm" className="border-slate-700 bg-slate-950 text-slate-200">
              <Link href="/live/zoom/hosting">My Zoom host classes</Link>
            </Button>
          )}
          <Button asChild variant="outline" size="sm" className="border-slate-700 bg-slate-950 text-slate-200">
            <Link href={`/live/zoom/${id}`} target="_blank" rel="noreferrer">
              Guest view
            </Link>
          </Button>
        </div>
          ...
          {session.status === 'live' || session.status === 'paused' ? (
            <Button
              ...
              onClick={() => {
                if (
                  confirm(
                    'End this Zoom session now? Guests will be disconnected. You can restart later without deleting.',
                  )
                ) {
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 space-y-3">
          ...
          {join ? (
            ...
            <p className="text-sm text-slate-400">
              Use Start / Join as host above. Zoom embedded UI includes mic, camera, and screen share.
            </p>
          )}
        </section>
